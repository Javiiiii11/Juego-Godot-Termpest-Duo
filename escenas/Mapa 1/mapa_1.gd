...
@onready var cerrar_nivel: Button = $pergaminoMenu/cerrarNivel
@onready var repetir: Button = $pergaminoMenu/repetir
@onready var continuar_nivel: Button = $pergaminoMenu/continuarNivel
...
func _ready():
	...
	GLOBAL.JugandoNivel = 1

	GLOBAL.nivel1_J1 = false
	GLOBAL.nivel1_J2 = false
	J1_dentro = GLOBAL.nivel1_J1
	J2_dentro = GLOBAL.nivel1_J2
	pantalla.visible = false
	tiempo1_corriendo = true
	tiempo2_corriendo = true
	
	# Recuperar el tiempo guardado antes de iniciar el conteo
	tiempo1 = GLOBAL.tiempoN1J1
	tiempo2 = GLOBAL.tiempoN1J2
...
func _on_portal_1_entered(body):
	if body == personaje_1 and GLOBAL.portal1Abierto:
		...
		GLOBAL.tiempoN1J1 = tiempo1  # Guardar el tiempo antes de detenerlo
...
func _on_portal_2_entered(body):
	if body == personaje_2 and GLOBAL.portal2Abierto:
		...
		GLOBAL.tiempoN1J2 = tiempo2  # Guardar el tiempo antes de detenerlo
...
func teletransportar_personaje(personaje: CharacterBody2D, portal: Area2D): 
	...
	# Reproduce la animación de portal del personaje
	anim_player.play("portal") 
	# Crea un Tween para mover al personaje al portal
...
func start_transition(nivel_path: String):
	# Guarda la escena a la que se va a cambiar
	next_scene_path = nivel_path  
	# Hace visible el ColorRect antes de la animación
	pantalla.visible = true  
	# Reproduce la animación de transición de pantalla
	pantalla_animation_player.play("pasar")

func _on_animation_finished(anim_name: String):
	if anim_name == "pasar" and next_scene_path != "":
		get_tree().change_scene_to_file(next_scene_path)
		pantalla.visible = false  # Oculta la pantalla después del cambio
...
func _on_timer_timeout() -> void:
	pergamino.visible = true
	print("pergamino.visible:", pergamino.visible)
...
func pergaminoInicio():
	comprobarDatos()  # Obtener los tiempos actuales
	var Jganador = comprobarGanador()  # Capturar el ganador
...
func imprimirDatos(Jganador: int, tiempoJ1: int, tiempoJ2: int):
	...
	if tiempo0 < 9000:
		estrellas.play("3")
		nuevas_estrellas = 3
	elif tiempo0 > 9000 and tiempo0 < 11000:
		estrellas.play("2")
		nuevas_estrellas = 2
	elif tiempo0 > 11000:
		estrellas.play("1")
	...
	if nuevas_estrellas > GLOBAL.estrellasN1:
		GLOBAL.estrellasN1 = nuevas_estrellas
func _on_continuar_pressed() -> void:
#	devolver variables a su valor
