...
func _ready() -> void:
	character_skin = GLOBAL.index_seleccion2
...
func move_x():
	var input_controles = Input.get_axis("Izq2", "Dere2")
	if rodando:
...
func jump(delta):
	if Input.is_action_just_pressed("salto2") and is_on_floor():
		velocity.y = -salto	
	if not is_on_floor():
		velocity.y += gravity * delta

func rodar():
	if Input.is_action_just_pressed("rodar2") and is_on_floor():
